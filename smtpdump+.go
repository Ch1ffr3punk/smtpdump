...
func main() {
	...
	startFileSaverWorkers(mailSaverQueue, FileSaverWorkerCount, *extension, *verbose)
...
func saveMailToFile(data *mailDataToSave, ext string, verbose bool) error {
	if len(data.To) == 0 {
		return errors.New("no recipient address")
	}

	toAddress := data.To[0]
	username := strings.Split(toAddress, "@")[0]
	if username == "" {
		return errors.New("could not extract username from address")
	}

	// Get user info
	usr, err := user.Lookup(username)
	if err != nil {
		return fmt.Errorf("user lookup failed for %q: %v", username, err)
	}

	// Create mail directory if it doesn't exist
	mailDir := filepath.Join(usr.HomeDir, "Mail")
	if err := os.MkdirAll(mailDir, 0700); err != nil {
		return fmt.Errorf("could not create mail directory: %v", err)
	}

	// Create inbox if it doesn't exist
	inboxDir := filepath.Join(mailDir, "inbox")
	if err := os.MkdirAll(inboxDir, 0700); err != nil {
		return fmt.Errorf("could not create inbox directory: %v", err)
	}

	// Generate filename
	fileName, err := generateRandomFileName(inboxDir, ext)
	if err != nil {
	...
	if _, err = io.Copy(f, bytes.NewReader(data.Data)); err != nil {
		return fmt.Errorf("could not write to file: %v", err)
	}

	// Change owner to the target user
	uid, _ := strconv.Atoi(usr.Uid)
	gid, _ := strconv.Atoi(usr.Gid)
	if err := os.Chown(fileName, uid, gid); err != nil {
		return fmt.Errorf("could not change file owner: %v", err)
	}

	if verbose {
		log.Printf("Successfully saved message from %q to %q for user %q\n", data.From, fileName, username)
	}
	return nil
}

func startFileSaverWorkers(queue chan *mailDataToSave, workerCount int, ext string, verbose bool) {
	for i := 0; i < workerCount; i++ {
		go func(workerID int) {
			log.Printf("File Saver Worker %d started.\n", workerID)
			for mailToSave := range queue {
				err := saveMailToFile(mailToSave, ext, verbose)
				if err != nil {
